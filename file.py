...
def check_file(input_dir, output_dir, release_version=True):
    ...
    file_list = [file_in for file_in in input_dir if not file_in.startswith('.')]
    if release_version is True:
        print("\nChecking valid files...")
        output_file_list = [file_out for file_out in output_dir if file_out.lower().endswith('.png')]
...
def error_messages(file_list, files_removed):
    ...
    for remove_this, reason in files_removed:
        ...
        if reason == 0:
            print(f" REMOVED : ('{remove_this}')   is not PNG file format")
        elif reason == 1:
            print(f" REMOVED : ('{remove_this}')   already exists")
        elif reason == 2:
            print(f" REMOVED : ('{remove_this}')   file unreadable")
