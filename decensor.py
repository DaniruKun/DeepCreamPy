...
class Decensor(QtCore.QThread):
    def __init__(self, text_edit=None, text_cursor=None, ui_mode=None):
        ...
        self.ui_mode = ui_mode if ui_mode is not None else args.ui_mode
    ...
    def decensor_all_images_in_folder(self):
        ...
        self.signals.total_ProgressBar_update_MAX_VALUE.emit(
            "set total progress bar MaxValue : " + str(len(file_names)), len(file_names))
        ...
        for n, file_name in enumerate(file_names, start=1):
            ...
            self.signals.update_progress_LABEL.emit("for-loop, \"for file_name in file_names:\"",
                                                    "Decensoring : " + str(file_name))
            ...
            if os.path.isfile(color_file_path) and color_ext.casefold() == ".png":
                custom_print("--------------------------------------------------------------------------")
                custom_print("Decensoring the image {}".format(color_file_path))
                try:
                    colored_img = Image.open(color_file_path)
                except:
                    custom_print("Cannot identify image file (" + str(color_file_path) + ")")
                ...
                if self.is_mosaic:
                    ...
                    for test_file_name in test_file_names:
                        ...
                        custom_print(
                            "Corresponding original, uncolored image not found in {}".format(color_file_path))
                        custom_print("Check if it exists and is in the PNG or JPG format.")
                # if we are doing a bar decensor
                else:
                    self.decensor_image_variations(colored_img, colored_img, file_name)
            else:
                custom_print("--------------------------------------------------------------------------")
                custom_print("Image can't be found: " + str(color_file_path))
        custom_print("--------------------------------------------------------------------------")
        ...
        self.signals.update_progress_LABEL.emit("finished",
                                                "Decensoring complete! Close this window and reopen DCP to start a new session.")
    ...
    def decensor_image_variation(self, ori, colored, variant_number, file_name):
        ...
        custom_print("Found {region_count} censored regions in this image!".format(region_count=len(regions)))

        if len(regions) == 0 and not self.is_mosaic:
            custom_print("No green regions detected! Make sure you're using exactly the right color.")
            return

        self.signals.signal_ProgressBar_update_MAX_VALUE.emit("Found {} masked regions".format(len(regions)),
                                                              len(regions))
        output_img_array = ori_array[0].copy()

        for region_counter, region in enumerate(regions, 1):
            self.signals.update_progress_LABEL.emit("\"Decensoring regions in image\"",
                                                    "Decensoring censor {}/{}".format(region_counter, len(regions)))
            ...
            self.signals.signal_ProgressBar_update_VALUE.emit(
                "{} out of {} regions decensored.".format(region_counter, len(regions)), region_counter)
            custom_print(
                "{region_counter} out of {region_count} regions decensored.".format(region_counter=region_counter,
        ...
        self.signals.update_progress_LABEL.emit("current image finished",
                                                "Decensoring of current image finished. Saving image...")

        if file_name is None:
            custom_print("Decensored image. Returning it.")
            return output_img
        else:
            ...
            custom_print("Decensored image saved to {save_path}!".format(save_path=save_path))
            return
